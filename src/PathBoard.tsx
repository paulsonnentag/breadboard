        ...
        {state.paths.map((path, index) => (
          ...
          >
            <PathView
              ...
              onDeletePath={() => actions.deletePathAtIndex(index)}
            />
          </div>
        ))}

        {state.paths.length !== 1 && <div className="h-[50px]"></div>}
        <div
          className="absolute bottom-0 h-[50px] w-full bg-gray-200 hover:bg-gray-300 transition-colors font-medium text-gray-500 flex items-center justify-center cursor-pointer"
          onClick={(e) => actions.addPath()}
        >
          &#43;
        </div>
      </div>
